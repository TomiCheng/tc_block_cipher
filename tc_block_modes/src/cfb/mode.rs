...
/// shorter one is right-aligned over zeros, so an empty one is all zeros.
...
impl<C: BlockCipher> BlockCipher for CfbBlockCipher<C> {
    ...
    fn process_block(&mut self, input: &[u8], output: &mut [u8]) -> Result<usize, Self::Error> {
        ...
        for ((output, input), keystream) in
            output[..segment].iter_mut().zip(input).zip(&self.keystream)
        {
    ...
    P: IvParams + ?Sized,
{
    ...
    fn init(
        ...
        // The engine's block size may have changed since new(); size the buffers from this init.
        self.iv.resize(block_size, 0);
        self.register.resize(block_size, 0);
        self.keystream.resize(block_size, 0);
        // Right-align a short IV over zeros (FIPS 81), so an empty IV is all zeros.
