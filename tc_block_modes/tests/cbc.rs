//! CBC: the NIST SP 800-38A F.2 AES-128 vectors, plus IV handling, reset and errors.
...
fn cbc_matches_the_nist_sp800_38a_aes128_vectors() {
    ...
    assert_vectors(
        &mut FixedAesCbc::new(AesEngine::new()),
        &params,
        &plaintext,
        &ciphertext,
    );
...
fn every_key_with_iv_container_initializes_the_same_cbc_state() {
    ...
    assert_eq!(
        process(&mut mode, CipherDirection::Encrypt, &fixed, plaintext),
        expected
    );
    let borrowed = KeyWithIvRef::new(&key, &iv);
    assert_eq!(
        process(&mut mode, CipherDirection::Encrypt, &borrowed, plaintext),
        expected
    );
    #[cfg(feature = "alloc")]
    {
        ...
        assert_eq!(
            process(&mut mode, CipherDirection::Encrypt, &owned, plaintext),
            expected
        );
    }
}
...
fn a_rejected_initialization_leaves_the_chaining_state_untouched() {
            ...
            + for<'a> BlockCipherInit<KeyWithIvRef<'a>, Error = BlockModeInitError<InitError>>,
    {
        ...
        // A bad key length: the engine rejects it, so the new IV must not take effect either.
        assert_eq!(
            mode.init(
                CipherDirection::Decrypt,
                &KeyWithIvRef::new(&key[..15], &[0xaa; 16])
            ),
