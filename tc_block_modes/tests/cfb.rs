//! CFB: the NIST SP 800-38A F.3 AES-128 vectors, plus IV padding, feedback size and errors.
...
/// F.3.7 CFB8-AES128, in eighteen one-byte segments.
...
fn a_short_iv_is_right_aligned_over_zeros_so_an_empty_iv_is_all_zero() {
        ...
        M: BlockCipher + for<'a> BlockCipherInit<KeyWithIvRef<'a>>,
    {
        ...
        let empty = process(
            &mut mode,
            encrypt,
            &KeyWithIvRef::new(&key, &[]),
            &plaintext,
        );
        let zero = process(
            &mut mode,
            encrypt,
            &KeyWithIvRef::new(&key, &[0; 16]),
            &plaintext,
        );
