//! CTR: the NIST SP 800-38A F.5 AES-128 vectors, plus the counter, IV lengths and errors.
...
use tc_block_modes::{
    BlockCipherMode, BlockModeError, BlockModeInitError, FixedCtrBlockCipher, KeyWithIvRef,
};

/// The F.5.1 initial counter block.
...
fn ctr_matches_the_nist_sp800_38a_aes128_vectors() {
    ...
    assert_vectors(
        &mut FixedAesCtr::new(AesEngine::new()),
        &params,
        &plaintext,
        &ciphertext,
    );
